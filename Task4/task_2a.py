...
# Team ID:			[ 1212 ]
# Author List:		[ B. Thirumurugan, Plaban Datta, Amudhapriyan E., Arunagiri T. ]
# Filename:			task_2a.py
# Functions:		
# Global variables:	
...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	##############	ADD YOUR CODE HERE	##############
	transformed_depth_image=np.array(vision_sensor_depth_image,dtype=np.float64)
	transformed_depth_image.resize([image_resolution[0],image_resolution[1]])
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	"""
	berries_dictionary = {}
	berries = ["Blueberry", "Strawberry", "Lemon"]
	global return_code
	global sample
	##############	ADD YOUR CODE HERE	##############
	...
	for contour in contours:
		...
		if(M['m00']!=0.0):
			x=round((int(M['m10']/M['m00'])),2)
			y=round((int(M['m01']/M['m00'])),2)
		for i in range(0,3):
			mask=cv2.inRange(hsv,np.array(lowrange[i]),np.array(highrange[i]))
			if(mask[y,x]==255):
				berrytype=berries[i]
		for i in range(1,imgres[0]):
			for j in range(1,imgres[1]):
				if(i==y and j==x):  
					z=round((transformed_depth_image[i][j]),2)
		berries_dictionary[berrytype].append((x,y,z))
...
def detect_berry_positions(berries_dictionary):
	...
	for i in berries:
		for j in berries_dictionary[i]:
			newx=-0.5+(j[0]/512)*(0.5+0.5)
			newy=-0.5+(j[1]/512)*(0.5+0.5)
			'''a=2.0/(2.0-0.01)
			b=2*0.01/(0.01-2)'''
			newz=round(((0.0141+j[2])*2),2)
			berry_positions_dictionary[i].append((round(newx,2),round(newy,2),newz))
			#m=2*j[2]-1
			#newz=0.01+m*2
