...
##############################################################
def gostraight(client_id,jointfl,jointfr,jointrl,jointrr):
...
def rotateright(client_id,jointfl,jointfr,jointrl,jointrr):
...
def rotateleft(client_id,jointfl,jointfr,jointrl,jointrr):
    sim.simxSetJointTargetVelocity(client_id,jointfl,-0.8,sim.simx_opmode_oneshot)
    sim.simxSetJointTargetVelocity(client_id,jointfr,0.8,sim.simx_opmode_oneshot)
    sim.simxSetJointTargetVelocity(client_id,jointrl,-0.8,sim.simx_opmode_oneshot)
    sim.simxSetJointTargetVelocity(client_id,jointrr,0.8,sim.simx_opmode_oneshot)
def zerovel(client_id,jointfl,jointfr,jointrl,jointrr):
...
def get_vision_sensor_image(client_id):
	...
	return_code,image_resolution,vision_sensor_image=sim.simxGetVisionSensorImage(client_id,vhandle,0,sim.simx_opmode_blocking)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	1. First convert the vision_sensor_image list to a NumPy array with qr_codes-type as uint8.
...
def detect_qr_codes(transformed_image):
	...
	for qrcode in qrcodes:
		qr_codes=qrcode.qr_codes.decode("utf-8")
		qr_codes=ast.literal_eval(qr_codes)
...
def init_setup(client_id):
	...
	wheel_joints=[jointfl,jointfr,jointrl,jointrr]
...
def nav_logic(client_id,target_points,transformed_image,wheel_joints):
	"""
	Purpose:
	---
	This function should implement your navigation logic. 
	"""
	jointfl,jointfr,jointrl,jointrr=wheel_joints
	data=detect_qr_codes(transformed_image)
	print(data)
	print(jointfl)
	print(target_points)
	zerovel(client_id,jointfl,jointfr,jointrl,jointrr)
	init=(0,0)
	for n in target_points:
		...
		if(y>yi and x>xi):
			...
			while(int(data[1])<y or data==(0,0)):
				data=detect_qr_codes(transformed_image)
                # print(data[1])
				gostraight(client_id,jointfl,jointfr,jointrl,jointrr)
			rotateright(client_id,jointfl,jointfr,jointrl,jointrr)
			time.sleep(15.2)
			while(int(data[0])<x or data==(0,0)):
				data=detect_qr_codes(transformed_image)               
				gostraight(client_id,jointfl,jointfr,jointrl,jointrr)
			data=detect_qr_codes(transformed_image)
			rotateleft(client_id,jointfl,jointfr,jointrl,jointrr)
			time.sleep(15.5)
		elif(y>yi and x<xi):
            # print("2nd loop")
			while(int(data[1])<y or data==(0,0)):
...
def task_3_primary(client_id, target_points):
	...
	for i in range(0,len(target_points)):
		for j in range(0,(len(target_points)-1)):
			if(target_points[j][1]>target_points[j+1][1]):
				store=target_points[j]
				target_points[j]=target_points[j+1]
				target_points[j+1]=store
				c+=1 
		if(c>0):
			c=0
		else:
	...
	nav_logic(client_id,target_points,transformed_image,wheel_joints)
